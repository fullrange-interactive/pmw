...
module.exports.createCanvasRenderingContext2D = function (canvas) {
  ...
  var imageFillColor = new Float32Array([1, 1, 1, 1]);
    ...
    function paintImage(vgHandle) {
      function paint() {
        if (globalAlpha === 1) {
          ...
          vg.setI(vg.VGParamType.VG_IMAGE_MODE, vg.VGImageMode.VG_DRAW_IMAGE_MULTIPLY);
          imageFillColor[3] = globalAlpha;
          vg.setParameterFV(fillPaint, vg.VGPaintParamType.VG_PAINT_COLOR, imageFillColor);
        }

        vg.drawImage(vgHandle);

        if (globalAlpha !== 1) {
          vg.setParameterFV(fillPaint, vg.VGPaintParamType.VG_PAINT_COLOR, effectiveColor);
        }
      }
  ...
  result.setLineDash = function (segments) {
    ...
    if (segments.length & 0x1 === 0) {
      ...
      for (i = 0; i < segments.length; i++) {
        dashList[i + segments.length] = lineDashPattern[i + segments.length] =
          dashList[i] = segments[i];
      }
    }

    vg.setFVOL(vg.VGParamType.VG_STROKE_DASH_PATTERN, lineDashPattern, 0, size);
  };
  ...
  function internalSetLineDash(state) {
    ...
    vg.setFVOL(vg.VGParamType.VG_STROKE_DASH_PATTERN,
               lineDashPattern, 0, dashList.length);
  }
