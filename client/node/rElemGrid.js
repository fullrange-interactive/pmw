...
)
{
  ...
  this.removeRelem = function(rElem, allIsClearing)
    {
      ...
      this.globalRelemList = this.globalRelemList.filter(function(value, index)
      {
        return value.instanceName != rElem.instanceName;
      });
  ...
  this.newRelem = function(baseX, baseY, sizeX, sizeY, className, zIndex, displayMode, data)
    {
      ...
      for (var cell in cellList)
      {
        ...
        this.relemGrid[cellList[cell].x][cellList[cell].y].relemList.sort(function(a, b)
        {
      ...
      this.globalRelemList.sort(function(a, b)
      {
        return a.z < b.z ? -1 : a.z > b.z ? 1 : 0
      });
  ...
  this.computePositions = function()
    {
      ...
      var cursor = {
        x: this.wrapper.base.x,
        y: this.wrapper.base.y
      };
      ...
      for (var y = 0; y < this.gridSizeY; y++)
      {
        ...
        for (var x = 0; x < this.gridSizeX; x++)
        {
          ...
          this.relemGrid[x][y].positions = {
            x: cursor.x,
            y: cursor.y
          };
          this.relemGrid[x][y].dimensions = {
            x: columnRatioList[x] * this.wrapper.width,
            y: rowRatioList[y] * this.wrapper.height
          };
  ...
  this.drawRelems = function(ctx)
  {
          ...
          for (var j in this.globalRelemList[i].cellList)
          {
            ...
            for (var k in this.globalRelemList[i].cellList[j])
            {
              ...
              for (var l in this.relemGrid[x][y].relemList)
              {
                 ...
                 */
                if (
                  this.globalRelemList[i].instanceName == this.relemGrid[x][y].relemList[l].instanceName || (this.globalRelemList[i].opaque && this.globalRelemList[i].z > this.relemGrid[x][y].relemList[l].z) || !this.relemGrid[x][y].relemList[l].isReady
                )
                  continue;
    ...
    for (var i in this.globalRelemList)
    {
      if (((this.globalRelemList[i].needRedraw || this.globalRelemList[i].redrawZones.length > 0) && this.globalRelemList[i].isReady) || this.forceFullDraw ||
