...
function(global) {
    ...
    pmw.Controllers.DrawphotoController = pmw.Controllers.AbstractController.extend({
        ...
        _initViews: function() {
            ...
            }), $(".page-drawphoto .selectionSize .selection-list").sizeChooser(this.changeSize), this.newCanvas(), $(".page-drawphoto .camera").prepend('<input type="file" capture="camera" accept="image/*" id="takePictureField">'), $(".page-drawphoto #takePictureField").on("change", this.gotPic), !("url" in window) && "webkitURL" in window && (window.URL = window.webkitURL)
        },
        newCanvas: function() {
